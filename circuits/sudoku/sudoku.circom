...
template Sudoku() {
    ...
    for (var i = 0; i < 9; i++) {
       for (var j = 0; j < 9; j++) {
           // Less or equal to 32 bits
           letnine[i][j] = LessEqThan(32);

           // given solved input
           letnine[i][j].in[0] <== solved[i][j];
           // constraint should be less than 9
           letnine[i][j].in[1] <== 9;

           // greater or equal to 32 bits
           getone[i][j] = GreaterEqThan(32);
           // given unsolved input
           getone[i][j].in[0] <== solved[i][j];
           // constraint should be greater than 1
           getone[i][j].in[1] <== 1;
    ...
    for (var i = 0; i < 9; i++) {
       for (var j = 0; j < 9; j++) {
            // isEqual comparator component
            // https://github.com/iden3/circomlib/blob/master/circuits/comparators.circom
            // https://twitter.com/sigridjin_eth/status/1548503350701789186
            ieBoard[i][j] = IsEqual();
            ieBoard[i][j].in[0] <== solved[i][j];
            ieBoard[i][j].in[1] <== unsolved[i][j];

            // ieBoard[i][j].out is a boolean that is true if unsolved[i][j] is equal to solved[i][j]

            izBoard[i][j] = IsZero();
            izBoard[i][j].in <== unsolved[i][j];

            // unsolved is 1) unequal to solved or 2) equal to zero at the first place
